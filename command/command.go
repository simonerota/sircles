...
func (s *CommandService) UpdateActivateMember(ctx context.Context, c *change.UpdateActivateMemberChange) (*change.UpdateActivateMemberResult, util.ID, error) {
	...
	member, err := readDBService.MemberActivate(ctx, curTlSeq, c.ID)
	if err != nil {
		return nil, util.NilID, err
	}
	if member == nil {
	...
	correlationID := s.uidGenerator.UUID("")
	causationID := s.uidGenerator.UUID("")
	command := commands.NewCommand(commands.CommandTypeRequestUpdateActivateMember, correlationID, causationID, callingMember.ID, commands.NewCommandRequestUpdateActivateMember(c, member.ID))
